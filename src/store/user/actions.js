...
const actions = {
  ...
  [Type.A_USER_CONFIRM_START]: ({commit, state, dispatch}, payload) => {
    return apiTools.req(api.userConfirm, payload, rsp => {
      commit(Type.M_USER_CONFIRM_SUCCESS, rsp)
      console.log('获取最新实名认证名单成功', rsp)
    }, msg => {
      console.log('获取最新实名认证名单失败', msg)
    })
  },
  ...
  [Type.A_USER_SEARCH]: ({commit, state, dispatch}, payload) => {
    console.log("A_USER_SEARCH payload",payload)
    return apiTools.post(api.search, payload, rsp => {
      commit(Type.M_SEARCH_SUCCESS, rsp)
    }, msg => {
      commit(Type.M_SEARCH_ERROR, msg)
    })
  },
